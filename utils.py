...
def get_model(name, num_classes=10, normalize_input=False):
    ...
    return model
def extract_penultimate_features(model, data, device):
    with torch.no_grad():
        features = []
        model.eval()

        for batch in torch.utils.data.DataLoader(data, batch_size=128, shuffle=False):
            batch = torch.tensor(batch, dtype=torch.float32)  # Ensure correct type
            if batch.dim() == 4 and batch.shape[-1] == 3:  # Fix dimension ordering if needed
            ...
            # Forward pass through layers to penultimate layer
            x = model.conv1(batch)
            x = model.block1(x)
            x = model.block2(x)
            x = model.block3(x)
            x = model.bn1(x)
            x = model.relu(x)  # Penultimate features
